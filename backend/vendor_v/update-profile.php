...
function uploadToCloudinary($file, $userId) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_POSTFIELDS => [
            ...
            "folder" => "profile-pictures/vendors/{$userId}"
        ]
    ]);
...
try {
    ...
    if (!empty($_FILES["profile_picture"]["tmp_name"])) {
        $profilePicture = uploadToCloudinary($_FILES["profile_picture"], $vendor_id);
    }
    ...
    $getUser->bind_param("i", $vendor_id);
    $getUser->execute();

    $updatedUser = $getUser->get_result()->fetch_assoc();

    response(true, "Vendor profile updated successfully", [
        "user" => $updatedUser
    ]);
} catch (Throwable $e) {
    error_log("Update vendor profile failed: " . $e->getMessage());
