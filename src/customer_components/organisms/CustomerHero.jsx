...
function CustomerHero() {
  ...
  const getImageUrl = (path, fallback = adobo) => {
  ...
  const arrivals = useMemo(
    () =>
      subcategories.slice(0, 5).map((item, index) => ({
        ...
        image: getImageUrl(
          item.image_path,
          fallbackImages[index % fallbackImages.length] || adobo,
        ),
  ...
  return (
    <section className="mt-3 w-full">
      <div className="grid gap-3 lg:grid-cols-[1fr_240px]">
        ...
        <aside className="min-w-0 overflow-hidden rounded-t-xl bg-white p-3 shadow-sm lg:h-[331px] lg:rounded-xl">
          ...
          <div className="w-full min-w-0 overflow-hidden">
            <div className="flex max-w-full gap-2 overflow-x-auto pb-1 no-scrollbar lg:h-[278px] lg:flex-col lg:overflow-y-auto lg:overflow-x-hidden lg:pr-1">
              {cards.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => openArrival(item)}
                  className="group relative h-28 w-[170px] shrink-0 overflow-hidden rounded-lg bg-slate-100 text-left shadow-sm lg:h-[121px] lg:w-full"
                >
                  <LazyLoadImage
                    src={item.image}
                    alt={item.title}
